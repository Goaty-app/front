        ...
        <Containers.Simple className="w-full h-full">
            { !isAuthenticated ?
                <Containers.Simple className="flex flex-col sm:flex-row gap-8 h-full p-4">
                    ...
                    <Cards.CardBtn onClick={() => setIsHealCareOpen(true)} color={'blue'} title={'Soin'} description={'Ajouter un soin.'}/>
                    <Cards.CardLink href={'/production'} color={'green'} title={'Production'} description={'Observer la production de votre élevage !'}/>
                    <Cards.CardLink href={'/birth'} color={'yellow'} title={'Nouvel arrivant'} description={'Gérer vos naissances ou nouvel arrivant ici!'}/>
                </Containers.Simple>
                :
            ...
            <HealthCareModal open={isHealthCareOpen} onOpenChange={setIsHealCareOpen} />
        </Containers.Simple>
    );
}
