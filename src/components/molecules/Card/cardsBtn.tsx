"use client";

import React from "react";
import "./card.css";
import { Btn, Containers, Typography } from "@/components/atoms";

interface CardLinkProps {
  onClick: () => void;
  color: "blue" | "green" | "orange" | "yellow";
  title: string;
  description: string;
}

const CardLink: React.FC<CardLinkProps & { children?: React.ReactNode }> = ({
  onClick,
  color,
  title,
  description,
  children,
}) => {
  return (
    <Btn variant="default" onClick={onClick} className="block w-full">
      <Containers.Simple
        className={`flex flex-col h-full w-full justify-center items-center rounded-lg p-6 shadow-md transition-transform hover:scale-105 ${
          color === "blue"
            ? "bg-blue light-text"
            : color === "green"
              ? "bg-green"
              : color === "orange"
                ? "bg-orange light-text"
                : "bg-yellow"
        }`}
      >
        <Typography.Title>{title}</Typography.Title>
        <Typography.Text className="text-sm opacity-85">
          {description}
        </Typography.Text>
        {children}
      </Containers.Simple>
    </Btn>
  );
};

export default CardLink;
