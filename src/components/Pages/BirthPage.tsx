...
const BirthPage: React.FC = () => {
    ...
    return (
        <Containers.Simple className="bg-layer-1 p-4 rounded shadow-md">
            <Typography.Title className="mb-4 dark-text">Gestion des Naissances</Typography.Title>
            ...
            <Box className="mt-4">
                ...
                {currentTab === 1 && (
                    <Form.BirthFromBreedingForm />
                )}
                {currentTab === 2 && (
                    <Form.BirthFromExternalForm />
                )}
                {currentTab === 3 && (
                    <Template.BirthHistoricTemplate />
                )}
            </Box>
        </Containers.Simple>
    );
