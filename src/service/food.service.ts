...
import type {
  ...
  FoodStockHistory,
  CreateFoodHistory,
} from "@/interface/foodStock.interface";
...
export const createFoodStock = async (
  herdId: string,
  foodStock: CreateFoodStock,
) => {
...
export const getFoodStockFoodStockHistory = async (foodStockId: number) => {
  return httpApi.get<FoodStockHistory[]>(
    `/food-stocks/${foodStockId}/food-stock-histories`,
  );
};

export const getFoodStockHistory = async (foodStockId: number) => {
  return httpApi.get<FoodStockHistory[]>(
    `/food-stock-histories/${foodStockId}`,
  );
};

export const addFoodStockHistory = async (
  foodStockId: number,
  history: CreateFoodHistory,
) => {
  return httpApi.post<FoodStockHistory>(
    `/food-stocks/${foodStockId}/food-stock-histories`,
    history,
    {
      headers: { "Content-Type": "application/json" },
...
export const deleteFoodStockHistory = async (
  foodStockId: number,
  historyId: number,
) => {
